...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < numARows; ++i)
  {
    for (int j = 0; j < numBColumns; ++j)
    {
      ...
      {
        tmp += hostA[i*numAColumns+h] * hostB[h*numBColumns+j];
      }
  ...
  M_mul <<<dimGrid,dimBlock>>> (deviceA,deviceB,deviceC,numARows,numAColumns,numBRows,numBColumns);
  ...
  for (int i = 0; i < numARows; i++)
  {
    for (int j = 0; j < numBColumns; j++)
    {
      if (fabs(resultRef[i*numBColumns + j] - hostC [i*numBColumns + j]) > 1e-5)
      {
